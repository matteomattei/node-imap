...
Connection.prototype.listRights = function(folderName, identifier, cb) {
  this._enqueue('LISTRIGHTS "' + escape(utf7.encode("" + folderName)) + '" "'+ escape(utf7.encode("" + identifier)) +'"', cb);
};


Connection.prototype.setAcl = function(folderName, username, permissions, cb) {
  this._enqueue(
    'SETACL "' + escape(utf7.encode("" + folderName)) + '" ' + username + " " + permissions,
    cb
  );
};

Connection.prototype.deleteAcl = function(folderName, username, cb) {
  this._enqueue('DELETEACL "' + escape(utf7.encode("" + folderName)) + '" ' + username, cb);
};
...
Connection.prototype._store = function(which, uids, cfg, cb) {
  ...
  if (
    ...
    throw new Error(
      (isFlags ? "Flags" : "Keywords") + " argument must be a string or a non-empty Array"
    );
  ...
  this._enqueue(
    which + "STORE " + uids + " " + modifiers + mode + "FLAGS.SILENT (" + items + ")",
    cb
  );
};
...
Connection.prototype._fetch = function(which, uids, options) {
  ...
  if (this.serverSupports("CONDSTORE") && !this._box.nomodseq) fetching.push("MODSEQ");
...
Connection.prototype._sort = function(which, sorts, criteria, cb) {
  ...
  sorts = sorts.map(function(c) {
    if (typeof c !== "string")
      throw new Error(
        "Unexpected sort criteria data type. " + "Expected string. Got: " + typeof criteria
      );
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === "bye") this._sock.end();
    ...
    let acl_res = info.text.trim().replace(/^".*" /, "");
    this._curReq.cbargs.push(acl_res.trim());
  } else if (type === "fetch") {
...
Connection.prototype._resTagged = function(info) {
  ...
  if (info.type === "no" || info.type === "bad") {
    ...
    if (req.type === "EXAMINE" || req.type === "SELECT") {
      this._box.readOnly =
        typeof info.textCode === "string" && info.textCode.toUpperCase() === "READ-ONLY";
    }
  ...
  self._enqueue("CAPABILITY", function() {
    ...
    if (self.state === "connected") {
      ...
      if (self.serverSupports("AUTH=XOAUTH") && self._config.xoauth) {
        ...
        self._caps = undefined;
        self._enqueue(
          'LOGIN "' + escape(self._config.user) + '" "' + escape(self._config.password) + '"',
          checkCaps
        );
      } else {
...
Connection.prototype._starttls = function() {
  ...
  this._enqueue("STARTTLS", function(err) {
    ...
    // Host name may be overridden the tlsOptions
    for (var k in this._config.tlsOptions) tlsOptions[k] = this._config.tlsOptions[k];
...
Connection.prototype._processQueue = function() {
  if (this._curReq || !this._queue.length || !this._sock || !this._sock.writable) return;
...
function buildSearchQuery(options, extensions, info, isOrChild) {
  ...
  for (var i = 0, len = options.length; i < len; ++i) {
    ...
    if (typeof criteria === "string") criteria = criteria.toUpperCase();
      ...
      throw new Error(
        "Unexpected search option data type. " + "Expected string or array. Got: " + typeof criteria
      );
    if (criteria === "OR") {
      if (args.length !== 2) throw new Error("OR must have exactly two arguments");
      ...
      switch (criteria) {
        ...
        case "SMALLER":
          ...
          if (isNaN(num)) throw new Error("Search option argument must be a number");
          searchargs += modifier + criteria + " " + args[0];
          break;
        case "HEADER":
          ...
          if (info && val[0] === "{") info.hasUTF8 = true;
          searchargs += modifier + criteria + ' "' + escape("" + args[0]) + '" ' + val;
          break;
        case "UID":
          if (!args)
            throw new Error("Incorrect number of arguments for search option: " + criteria);
        ...
        default:
          ...
          if (!validateUIDList(seqnos, true)) {
            if (seqnos.length === 0) throw new Error("Empty sequence number list");
