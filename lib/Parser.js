...
var CH_LF = 10,
  ...
  RE_LISTCONTENT = /^\((.*)\)$/,
  RE_LITERAL = /\{(\d+)\}$/,
  RE_UNTAGGED = /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|ACL|METADATA|MYRIGHTS|LSUB|SEARCH|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i,
...
function parseBoxList(text, literals) {
  ...
  // quotes
  text = text.replace(RE_LIST_REPONSE, function(
    match,
    p1,
    boxName,
    offset,
...
function parseStatus(text, literals) {
  // We need to make sure that the box name in the response is surrounded in
  // quotes
  text = text.replace(RE_STATUS_RESPONSE, function(
    match,
    boxName,
    p2,
    offset,
