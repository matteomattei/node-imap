...
var CH_LF = 10,
  ...
  RE_LISTCONTENT = /^\((.*)\)$/,
  RE_LITERAL = /\{(\d+)\}$/,
  RE_UNTAGGED = /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|ACL|LSUB|SEARCH|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i,
  ...
  RE_LWS_ONLY = /^[ \t]*$/;
(RE_LIST_REPONSE = /^(\(.*\) (?:"."|NIL) )(.+)$/), (RE_STATUS_RESPONSE = /^(.*)( \(.+\))$/);
...
Parser.prototype._parse = function(data) {
  ...
  if (this._literallen > 0) {
    if (this._body) {
      ...
      if (datalen > this._literallen) this._literals.push(data.slice(0, this._literallen));
...
Parser.prototype._resUntagged = function() {
  ...
  if ((m = RE_BODYLITERAL.exec(this._buffer))) {
    ...
    if (type === "flags" || type === "search" || type === "capability" || type === "sort") {
      if (m[5]) {
    ...
    else if (type === "fetch") val = parseFetch.call(this, m[5], this._literals, num);
...
function parseId(text, literals) {
  ...
  for (var i = 0, len = r[0].length; i < len; i += 2) id[r[0][i].toLowerCase()] = r[0][i + 1];
  ...
  for (n = 0; n < 3; ++n) {
    if (r[n]) {
      ...
      for (i = 0, len = r[n].length; i < len; ++i) {
        ...
        for (j = 2, len2 = ns.length; j < len2; j += 2) nsobj.extensions[ns[j]] = ns[j + 1];
...
function parseStatus(text, literals) {
  ...
  for (var i = 0, len = r[1].length; i < len; i += 2) attrs[r[1][i].toLowerCase()] = r[1][i + 1];
  return {
...
function parseFetch(text, literals, seqno) {
  ...
  for (var i = 0, len = list.length, key, val; i < len; i += 2) {
    ...
    if (key === "envelope") val = parseFetchEnvelope(val);
      ...
      val = "" + val[0];
    else if (key === "body" || key === "bodystructure") val = parseBodyStructure(val);
...
function parseBodyStructure(cur, literals, prefix, partID) {
  ...
  if (prefix === undefined) {
    ...
    if (Array.isArray(cur[0])) {
      ...
      if (
        (part.type === "text" || (part.type === "message" && part.subtype === "rfc822")) &&
        partLen > next
      )
        part.lines = cur[next++];
...
function parseStructExtra(part, partLen, cur, next) {
  ...
  if (partLen > next) {
    ...
    if (cur[next] !== null) part.language = Array.isArray(cur[next]) ? cur[next] : [cur[next]];
  ...
  for (var i = start, len = o.str.length; i < len; ++i) {
    if (!inQuote) {
      if (isBody) {
        ...
        if ((o.str[i] === ")" || (useBrackets && o.str[i] === "]")) && !isTop) return i;
        lastPos = i;
...
function decodeWords(str, state) {
  ...
  while ((m = RE_ENCWORD.exec(str))) {
    seq = {
      consecutive: pendoffset > -1 ? RE_LWS_ONLY.test(str.substring(pendoffset, m.index)) : false,
  ...
  for (i = 0, leni = replaces.length; i < leni; ++i) {
    ...
    if (m.encoding === "q") {
      ...
      next = replaces[i + 1];
      if (next && next.consecutive && next.encoding === m.encoding && next.charset === m.charset) {
        // we use the next base64 chunk, if any, to determine the integrity
        // of the current chunk
        next.buf = new Buffer(next.chunk, "base64");
      }
    }
    decodeBytes(bytes, m.charset, m.index, m.length, m.pendoffset, state, next && next.buf);
  }
  ...
  for (i = state.replaces.length - 1; i >= 0; --i) {
    ...
    if (Array.isArray(seq)) {
      for (j = 0, lenj = seq.length; j < lenj; ++j) {
        str = str.substring(0, seq[j].fromOffset) + seq[j].val + str.substring(seq[j].toOffset);
      }
    } else {
      str = str.substring(0, seq.fromOffset) + seq.val + str.substring(seq.toOffset);
    }
  }
...
function parseHeader(str, noDecode) {
  ...
  if (!noDecode) {
    ...
    for (h in header) {
      hvs = header[h];
      for (i = 0, len = header[h].length; i < len; ++i) hvs[i] = decodeWords(hvs[i], state);
    }
  }
